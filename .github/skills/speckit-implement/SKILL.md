...
description: Ejecuta el plan de implementación procesando y completando todas las
  tareas definidas en `tasks.md`.
...
Ejecuta `.specify/scripts/powershell/check-prerequisites.ps1 -Json -RequireTasks -IncludeTasks` desde la raíz del repositorio y analiza `FEATURE_DIR` y `AVAILABLE_DOCS`.
...
Si `tasks.md` está incompleto o no existe, sugiere ejecutar `/speckit-tasks` para regenerarlo.
